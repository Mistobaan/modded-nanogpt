...
TRAIN_DATASET = os.path.join(
    os.path.dirname(__file__), "./data/fineweb10B/fineweb_train_000001.bin"
)
        ...
        for group in self.param_groups:
            ...
            for base_i in range(0, len(params), world_size):
                ...
                if base_i + rank < len(params):
                    ...
                    grad = p.grad
                    scale = max(1, p.size(-2) / p.size(-1)) ** 0.5 * getattr(
                        p, "lr_mul", 1.0
                    )
                    eff_lr = lr_tensor * scale
                    eff_weight_decay = (
                        lr_tensor * weight_decay_tensor * getattr(p, "wd_mul", 1.0)
                    )
                    state = self.state[p]
                    if len(state) == 0:
                    ...
                    if shard.numel() == 0:
                        ...
                        if "exp_avg" not in state:
                            state["exp_avg"] = torch.zeros(
                                shard_shape, dtype=p.dtype, device=p.device
                            )
                        if "exp_avg_sq" not in state:
...
class GPT(nn.Module):
    def __init__(
        ...
        self.checkpoint_blocks = tuple(
            i for i in range(num_layers) if 1 < i < num_layers - 2
        )
...
for opt in optimizers:
    for group in opt.param_groups:
        ...
        group["lr_tensor"] = torch.tensor(
            group["lr"], dtype=torch.float32, device=param_device
        )
        group["weight_decay_tensor"] = torch.tensor(
            group.get("weight_decay", 0.0), dtype=torch.float32, device=param_device
        )
        if "momentum" in group:
            group["momentum_tensor"] = torch.tensor(
                group["momentum"], dtype=torch.float32, device=param_device
            )
...
capture_loader = distributed_data_generator(
    args.train_files, world_size * args.train_seq_len, align_to_bos=True
)
